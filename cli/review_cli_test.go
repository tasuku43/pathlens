...
func TestReviewCLIQueueAndDiffGuideAgentReview(t *testing.T) {
	...
	if queuePayload.Summary.SuggestedCommands[1].Command != "comments work" || queuePayload.Summary.SuggestedCommands[1].Intent != "wait_for_gui_feedback" || !containsString(queuePayload.Summary.SuggestedCommands[1].Args, "--actor") || !containsString(queuePayload.Summary.SuggestedCommands[1].Args, "codex:test") || !containsString(queuePayload.Summary.SuggestedCommands[1].Args, "--wait") || !containsString(queuePayload.Summary.SuggestedCommands[1].Args, "--loop") || !containsString(queuePayload.Summary.SuggestedCommands[1].Args, "--idle-on-change") || containsString(queuePayload.Summary.SuggestedCommands[1].Args, "--full") || !containsString(queuePayload.Summary.SuggestedCommands[1].Args, server.URL) || queuePayload.Summary.SuggestedCommands[1].ClientEventID == "" {
		t.Fatalf("review queue suggestions = %#v", queuePayload.Summary.SuggestedCommands)
	}
	...
	for _, text := range []string{
		...
		"vivi comments work --actor codex:test --wait --loop --idle-events --idle-on-change --url " + server.URL + " --client-event-id review-queue:codex:test:work --json --activity-limit 20 --comment-limit 10",
	} {
...
func TestReviewCLIHumanSuggestionsUseInvokedExecutable(t *testing.T) {
	...
	if strings.Contains(humanText, "comments work --actor codex:test --wait --loop --idle-events --idle-on-change --full") {
...
func TestReviewHelpTextSurfacesAgentQuickPath(t *testing.T) {
	...
	for _, text := range []string{
		...
		"vivi comments work --actor <actor> --wait --loop --idle-events --idle-on-change --json",
